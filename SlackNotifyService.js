...
SlackNotifyService.notifyDiffResult = function (meta) {
  ...
  LoggerUtil.start(fn, {
    ...
    hasSheetLinks: !!(meta && meta.sheetLinks)
  });
...
SlackNotifyService.notifyDiffResultDetailOnly = function (meta) {
  ...
  try {
    ...
    const limited = (chunks.length > maxItems) ? chunks.slice(0, maxItems) : chunks;

    const bodyLines = [];
    limited.forEach(c => (c.lines || []).forEach(l => bodyLines.push(l)));

    const warnLine = (chunks.length > maxItems)
    ...
    SlackNotifyService.post_(webhookUrl, Object.assign({}, payloadBase, { text: text }));
...
SlackNotifyService.notifyDiffResultSummaryWithUrlAndDetails = function (meta) {
  ...
  LoggerUtil.start(fn, {
    ...
    curr: meta && meta.curr,
    summary: meta && meta.summary,
    hasUrl: !!(meta && meta.diffSheets && meta.diffSheets.length),
    channel: meta && meta.channel
  });
  ...
  try {
    ...
    SlackNotifyService.post_(webhookUrl, Object.assign({}, payloadBase, { text: detailText }));
 ...
 * @returns {string}
 */
SlackNotifyService.buildSheetNameWithOptionalLink_ = function (name, linkObj) {
  const fn = 'SlackNotifyService.buildSheetNameWithOptionalLink_';
  LoggerUtil.start(fn, { name: name, hasLink: !!(linkObj && linkObj.url) });
...
SlackNotifyService.buildSummaryLines_ = function (meta) {
  ...
  try {
    ...
    const prev = SlackNotifyService.buildSheetNameWithOptionalLink_(
      meta.prev,
      meta.sheetLinks && meta.sheetLinks.prev ? meta.sheetLinks.prev : undefined
    );
    const curr = SlackNotifyService.buildSheetNameWithOptionalLink_(
      meta.curr,
      meta.sheetLinks && meta.sheetLinks.curr ? meta.sheetLinks.curr : undefined
    );
...
SlackNotifyService.buildDetailBodyChunksByDiff_ = function (meta) {
  ...
  LoggerUtil.start(fn, { diffs: meta && meta.diffs ? meta.diffs.length : 0 });
 ...
 * @returns {string}
 */
SlackNotifyService.buildSingleDetailPostText_ = function (opt) {
  const fn = 'SlackNotifyService.buildSingleDetailPostText_';
  LoggerUtil.start(fn, {
    prefix: opt && opt.prefixLines ? opt.prefixLines.length : 0,
    body: opt && opt.bodyLines ? opt.bodyLines.length : 0,
    maxLines: opt && opt.maxLines,
    maxChars: opt && opt.maxChars,
    hasWarn: !!(opt && opt.warnLine),
    wrapAll: !!(opt && opt.wrapAllInCodeBlock)
  });
  ...
  try {
    ...
    const wrapAll = !!(opt && opt.wrapAllInCodeBlock);
    ...
    if (wrapAll) {
      ...
      prefixLines.forEach(l => lines.push(SlackNotifyService.sanitizeSlackLine_(String(l))));
      ...
      if (bodyLines.length === 0) {
        ...
        bodyLines.forEach(l => lines.push(SlackNotifyService.sanitizeSlackLine_(l)));
      }
 ...
 */
SlackNotifyService.buildRepInfo_ = function (row, headers) {
  ...
  try {
    ...
    if (typeof DIFF_GIT_REP_COLUMNS === 'undefined' || !Array.isArray(DIFF_GIT_REP_COLUMNS)) {
      return '';
    }
