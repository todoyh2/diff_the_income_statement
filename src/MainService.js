 ...
 */
MainService.runSnapshotAndDiff = (outputMode, notifySlack) => {
	const fn = "MainService.runSnapshotAndDiff";
	LoggerUtil.start(fn, { outputMode, notifySlack });

	try {
		const currSnapshotName =
			SnapshotService.createOrUpdateDailySnapshot("manual");
		const prevSnapshotName =
			MainService.findPreviousSnapshot_(currSnapshotName);
 ...
 */
MainService.runDiffLatest = (outputMode, notifySlack) => {
	...
	try {
		...
		MainService.runDiffInternal_(pair.prev, pair.curr, outputMode, notifySlack);
	} catch (e) {
		LoggerUtil.error(e);
		throw e;
	} finally {
 ...
 * @param {boolean} notifySlack Slack 通知有無（sheet出力ありルート）
 */
MainService.runDiffManual = (prevName, currName, outputMode, notifySlack) => {
	const fn = "MainService.runDiffManual";
	LoggerUtil.start(fn, { prevName, currName, outputMode, notifySlack });

	try {
		MainService.runDiffInternal_(prevName, currName, outputMode, notifySlack);
	} catch (e) {
		LoggerUtil.error(e);
		throw e;
	} finally {
 ...
 */
MainService.runDiffForSlackOnlyManual = (
	prevSheetName,
	currSheetName,
	channel,
) => {
	...
	try {
		...
		if (DIFF_USE_GIT_LIKE) {
			...
			const sortKeys = (
				Array.isArray(DIFF_GIT_SORT_KEYS) ? DIFF_GIT_SORT_KEYS : []
			)
				.map((x) => x?.col)
				.filter(Boolean);

			diffs = DiffCore.buildDiffGitLike(prevRows, currRows, headers, {
				sortKeys: sortKeys,
			});
		} else {
 ...
 */
MainService.buildDiffSummary_ = (diffs) => {
...
MainService.runDiffInternal_ = (
	...
	try {
		...
		LoggerUtil.info(
			`diff 取得行数: prev=${prevRows.length}, curr=${currRows.length}`,
		);

		const diffs = DIFF_USE_GIT_LIKE
			? DiffCore.buildDiffGitLike(prevRows, currRows, headers, {
					sortKeys: DIFF_GIT_SORT_KEYS,
				})
			: DiffCore.buildDiff(prevRows, currRows, headers);
		const summary = MainService.buildSummary_(diffs);
		LoggerUtil.info(
			`diff 要約: MODIFY=${summary.mod} | ADD=${summary.add} | DELETE=${summary.del}`,
		);
		...
		if (outputMode?.visual) {
			const visualUrlMaybe = DiffVisualService.generate(
				prevName,
				currName,
				diffs,
				headers,
			...
			const visualSheetName = MainService.buildDiffSheetNameCompare_(
				prevName,
				currName,
			);
			const visualUrl =
				MainService.findSheetUrlByName_(visualSheetName) ||
				visualUrlMaybe ||
				"";
			...
			LoggerUtil.info(
				`差分比較 出力完了: sheet=${visualSheetName} url=${visualUrl}`,
			);
		}
		...
		if (outputMode?.detail) {
			...
			const detailSheetName = MainService.buildDiffSheetNameDetail_(
				prevName,
				currName,
			);
			...
			LoggerUtil.info(
				`変更一覧 出力完了: sheet=${detailSheetName} url=${detailUrl}`,
			);
		}

		// --- Slack通知（sheet出力あり） ---
		if (notifySlack) {
			const wantsDetailInline = !!outputMode?.detail;
			...
			if (wantsDetailInline) {
				...
				SlackNotifyService.notifyDiffResult({
					...
					diffSheetUrl: diffSheets[0]?.url ? diffSheets[0].url : "",
				});
 ...
 * @returns {{add:number,del:number,mod:number}}
 */
MainService.buildSummary_ = (diffs) => {
 ...
 * @returns {string}
 */
MainService.buildDiffSheetNameCompare_ = (prevName, currName) =>
	`diff_${prevName}_vs_${currName}`;
 ...
 * @returns {string}
 */
MainService.buildDiffSheetNameDetail_ = (prevName, currName) => {
 ...
 * @returns {string}
 */
MainService.findSheetUrlByName_ = (sheetName) => {
 ...
 */
MainService.findPreviousSnapshot_ = (currentName) => {
 ...
 */
MainService.findLatestTwoSnapshots_ = () => {
